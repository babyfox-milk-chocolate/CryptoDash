...
> Персональный трекер криптовалют: дашборд с живыми ценами, графики истории цен и параллельная агрегация данных из CoinGecko
...
FastAPI был выбран для асинхронного сбора данных из внешнего API. Когда нужно получить несколько независимых ресурсов (рыночные данные + история цен), последовательные запросы ждут друг друга, а `asyncio.gather` выполняет их одновременно:
